...
A private local **HTTPS** reverse proxy that gives your browser a secure context for VS Code / VSCodium / Lingma / Qoder web servers (`serve-web` mode) — no cloud tunnel required.
...
- **Supports all major VS Code variants** — `vscode`, `vscodium`, `lingma`, `qoder`, or any custom binary.
...
# Launch a VSCodium backend automatically (HTTPS is on by default)
...
# Proxy without HTTPS
lengcat-vst --backend-type vscodium --backend-port 8000 --no-https
```
...
Fields available:
- **Backend type** — `vscode`, `vscodium`, `lingma`, `qoder`, or `custom` (with custom executable path).
...
```bash
# Via CLI
lengcat-vst --backend-type vscode --backend-port 8080 --extension-host-only

# Via dashboard: tick "Extension-host only" in the New Session dialog
```

If the primary binary (`code` / `codium`) is not found on `PATH`, the proxy automatically looks for a server binary in the home directory:
...
Options:
  ...
  --backend-type <type>      vscode | vscodium | lingma | qoder | custom
                                                             (default: vscodium)
