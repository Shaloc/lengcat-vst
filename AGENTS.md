...
> Generated by the `/teach-impeccable` skill from `pbakaus/impeccable`.

### Users
Developers and DevOps engineers who manage multiple VS Code / code-server
sessions through a single browser page.  They access the dashboard from a
desktop browser (occasionally a tablet), often over SSH tunnels or local
networks.  The primary job-to-be-done is launching, switching between, and
monitoring remote coding sessions without exposing them to the public internet.

### Brand Personality
Technical · Reliable · Clean

### Aesthetic Direction
- **Theme**: Catppuccin Mocha (dark default) with a Catppuccin Latte light
  override.  Tinted neutrals, never pure black/white.
- **Typography**: System UI font stack — fast, native-looking, no external
  font loading.
- **Tone**: Utilitarian developer tool — minimal decoration, clear status
  indicators, no marketing fluff.
- **Anti-references**: Glassmorphism, gradient text, neon-on-dark "AI slop"
  aesthetics.  This is a professional tool, not a landing page.

### Design Principles
1. **Functional first** — every visual element must serve a clear purpose.
2. **Clear status** — session state (running / stopped / error / starting)
   must always be instantly visible via the colour-coded dots.
3. **Minimal friction** — creating and switching sessions should require as
   few clicks as possible; modals and confirmations only when destructive.
4. **Accessible** — keyboard-navigable (focus-visible outlines, Escape to
   close modals), reduced-motion support, ARIA labels on icon-only buttons.
